(() => {
  ...
  const MONTH_NAMES_NONE = null; // placeholder to keep linter quiet

  /* ---------- state ---------- */
  let entries = loadEntries();
  let payments = loadPayments();
  let viewedYear, viewedMonth; // viewedMonth: 0-11
  let editingId = null;
  ...
  const rowsUnpaid = document.getElementById("rowsUnpaid");
  const rowsPaid = document.getElementById("rowsPaid");
  const emptyUnpaid = document.getElementById("emptyUnpaid");
  const emptyPaid = document.getElementById("emptyPaid");
  ...
  /* ---------- rendering ---------- */
  function render() {
    monthLabel.textContent = `${viewedYear}年 ${viewedMonth + 1}月`;
    ...
    const active = entries.slice().sort((a, b) => a.day - b.day);
    ...
    active.forEach((entry) => {
      ...
      const el = buildRow(entry, paid, overdue, key);
      if (paid) paidRows.push(el);
      else unpaidRows.push(el);
    });
    ...
    rowsUnpaid.replaceChildren(...unpaidRows);
    rowsPaid.replaceChildren(...paidRows);
    emptyUnpaid.hidden = unpaidRows.length > 0;
    emptyPaid.hidden = paidRows.length > 0;
    countUnpaid.textContent = String(unpaidRows.length);
    countPaid.textContent = String(paidRows.length);

    const totalCount = active.length;
    const donePct = totalCount ? Math.round((paidRows.length / totalCount) * 100) : 0;
    progressFill.style.width = donePct + "%";
    progressText.textContent = `処理済み ${paidRows.length} / ${totalCount} 件`;
  }
  ...
  function buildRow(entry, paid, overdue, key) {
    ...
    day.className = "row-day mono";
    day.textContent = String(entry.day).padStart(2, "0");
    ...
    meta.textContent = entry.category + (entry.memo ? " ・ " + entry.memo : "") + (overdue ? " ・ 期限超過" : "");
    main.appendChild(name);
    main.appendChild(meta);

    const amount = document.createElement("div");
    amount.className = "row-amount mono";
    amount.textContent = (entry.kind === "income" ? "+" : "−") + yen(entry.amount).replace("¥", "¥");

    const stamp = document.createElement("button");
    stamp.className = "stamp-btn";
    stamp.type = "button";
    stamp.textContent = "済";
    stamp.setAttribute("aria-label", paid ? "未処理に戻す" : "処理済みにする");
